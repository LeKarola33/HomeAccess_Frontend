...
const getAuthState = () => {
  try {
    return JSON.parse(localStorage.getItem('homeaccess-auth'))?.state || {};
  } catch { return {}; }
};
...
const saveNewToken = (accessToken) => {
  try {
    ...
    localStorage.setItem('homeaccess-auth', JSON.stringify(store));
  } catch {}
};

// Auto-refresh: si el access token expiró, intenta renovarlo con el refresh token
const refreshAccessToken = async () => {
  const refreshToken = getRefresh();
  if (!refreshToken) throw new Error('Sesión expirada. Por favor inicia sesión nuevamente.');
...
const api = async (path, opts = {}, retry = true) => {
  ...
  if (res.status === 401 && retry) {
    try {
      ...
      res   = await fetch(`${BASE_URL}${path}`, { headers: authH(token), ...opts });
    } catch (err) {
...
const VehicleFormModal = ({ vehicle, onClose, onSaved }) => {
  ...
  // Cargar residentes al seleccionar unidad
  useEffect(() => {
    if (!form.unit_id) { setResidents([]); set('propietario_id', ''); return; }
    setLoadingRes(true);
    api(`/units/${form.unit_id}`)
      .then((r) => {
        ...
        (u.residentes || []).forEach((res) => {
          if (!lista.find((x) => x._id === res._id)) lista.push(res);
        });
        setResidents(lista);
        if (lista.length === 1) set('propietario_id', lista[0]._id);
        else if (!isEdit)       set('propietario_id', '');
      })
      .catch(() => setResidents([]))
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <input
            ...
            maxLength={6}
            disabled={isEdit}
            className={`${inp} font-mono font-bold text-base tracking-widest
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input value={form.marca} onChange={(e) => set('marca', e.target.value)}
              placeholder="Ej: Chevrolet" className={inp} />
          </div>
          <div>
            <label className={lbl}>Modelo</label>
            <input value={form.modelo} onChange={(e) => set('modelo', e.target.value)}
              placeholder="Ej: Spark GT" className={inp} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input value={form.color} onChange={(e) => set('color', e.target.value)}
              placeholder="Ej: Rojo" className={inp} />
          </div>
          <div>
            <label className={lbl}>Año</label>
            <input type="number" value={form.anio} onChange={(e) => set('anio', e.target.value)}
              placeholder={String(new Date().getFullYear())}
              min="1970" max={new Date().getFullYear() + 1} className={inp} />
          </div>
        </div>

        {/* Asignación — selects desde la BD */}
        {!isEdit && (
          <div className="p-4 bg-slate-50 rounded-xl space-y-4 border border-slate-100">
            ...
            <div>
              ...
              {loadingUnits ? (
                <div className="flex items-center gap-2 px-3.5 py-2.5 border border-slate-200
                  rounded-xl text-sm text-slate-400 bg-white">
            ...
            <div>
              ...
              {!form.unit_id ? (
                <div className="px-3.5 py-2.5 border border-slate-200 rounded-xl
                  text-sm text-slate-400 bg-slate-50">
                  Selecciona primero la unidad
                </div>
              ) : loadingRes ? (
                <div className="flex items-center gap-2 px-3.5 py-2.5 border border-slate-200
                  rounded-xl text-sm text-slate-400 bg-white">
                  <Spinner size={4} /> Cargando residentes...
                </div>
              ) : residents.length === 0 ? (
                <div className="px-3.5 py-2.5 border border-amber-200 rounded-xl
                  text-sm text-amber-600 bg-amber-50">
                  ⚠️ Esta unidad no tiene residentes registrados
                </div>
              ) : (
                <select value={form.propietario_id}
...
const DeleteModal = ({ vehicle, onClose, onDeleted }) => {
  ...
  const handle = async () => {
    setLoading(true);
    try {
      await api(`/vehicles/${vehicle._id}`, { method: 'DELETE' });
...
const VehicleRow = ({ v, isAdmin, onEdit, onDelete, idx }) => {
  ...
  return (
    <tr className={`group transition-colors
      ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/40'} hover:bg-blue-50/30`}>
      <td className="px-5 py-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#6366f1]/5 border border-[#6366f1]/10
      ...
      <td className="px-5 py-4">
        {v.parqueadero_id ? (
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#6366f1]/5
            border border-[#6366f1]/15 rounded-xl">
...
export default function VehiclesPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f4f6f9]">
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          <div className="col-span-2 sm:col-span-1 bg-white rounded-2xl p-4 shadow-sm
            ...
            <div>
              <p className="text-2xl font-bold text-[#1a2035]">{totalAll}</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="flex flex-wrap items-center gap-3 p-5 border-b border-slate-100">
            <div className="relative min-w-[180px] max-w-xs flex-1">
              ...
              <input value={search}
                ...
                className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 text-sm
                  outline-none focus:border-[#6366f1] focus:ring-2 focus:ring-[#6366f1]/10
            ...
            <select value={spotFilter} onChange={(e) => setSpotFilter(e.target.value)}
              className="px-3 py-2.5 rounded-xl border border-slate-200 text-sm
                text-slate-600 outline-none focus:border-[#6366f1] bg-white">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-24 gap-3">
              <Spinner size={10} /><p className="text-sm text-slate-400">Cargando vehículos...</p>
            </div>
          ) : vehicles.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {vehicles.map((v, i) => (
                      <VehicleRow key={v._id} v={v} idx={i} isAdmin={isAdmin}
                        onEdit={setFormModal} onDelete={setDeleteModal} />
                    ))}
                  </tbody>
                </table>
              </div>
      ...
      {deleteModal && (
        <DeleteModal vehicle={deleteModal}
