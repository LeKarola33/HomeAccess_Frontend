 ...
 * ===========================================
 * Acceso exclusivo para: admin, portero, vigilante.
 * Ruta: /admin/login
 * Si un residente intenta entrar aquí, se rechaza con mensaje claro.
 */
...
const ForgotPasswordView = ({ onBack }) => {
  ...
  const onSubmit = async ({ email }) => {
    ...
    setIsLoading(true);
    try {
      // Llamada a tu API de recuperación — ajusta el endpoint según tu backend
      // await forgotPassword(email);
      await new Promise((r) => setTimeout(r, 1200)); // simulación de llamada
      setEmailSent(true);
    } catch (error) {
      setServerError(
        ...
        <div className="relative space-y-5">
          ...
          <div className="space-y-2 pt-2">
            {[
              ...
              { role: 'Vigilante',     desc: 'Monitoreo y registro de accesos', color: '#fbbf24' },
            ].map(({ role, desc, color }) => (
